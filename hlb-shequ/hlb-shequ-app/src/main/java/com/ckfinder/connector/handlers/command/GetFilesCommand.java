...
public class GetFilesCommand extends XMLCommand {
	...
	@Override
	protected void createXMLChildNodes(final int errorNum, final Element rootElement) throws ConnectorException {
	...
	protected int getDataForXml() {
		...
		this.fullCurrentPath = configuration.getTypes().get(this.type).getPath() + this.currentFolder;

		if (!AccessControlUtil.getInstance(configuration).checkFolderACL(this.type, this.currentFolder, this.userRole, AccessControlUtil.CKFINDER_CONNECTOR_ACL_FILE_VIEW)) {
		...
		try {
			...
			List<String> originalFiles = FileUtils.findChildrensList(dir, false);

			// 过滤文件名,用用户的source和用户id的形式过滤
			User user = UserUtils.getUser();
			String prefix = Encodes.encodeBase62(user.getSource() + user.getId());
			for (String str : originalFiles) {
	...
	private void filterListByHiddenAndNotAllowed() {
		...
		for (String file : this.files) {
			if (FileUtils.checkFileExtension(file, this.configuration.getTypes().get(this.type), this.configuration, false) == 0
	...
	private String createThumbAttr(final File file) {
		File thumbFile = new File(configuration.getThumbsPath() + File.separator + type + this.currentFolder, file.getName());
		if (thumbFile.exists()) {
			return file.getName();
	...
	private boolean isAddThumbsAttr() {
		return configuration.getThumbsEnabled() && (configuration.getThumbsDirectAccess() || isShowThumbs());
	}
