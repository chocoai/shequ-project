package com.haolinbang.common.persistence;
...
import com.haolinbang.modules.sys.utils.HlbregisterUtils;
...
public class Page<T> {
	...
	private int pageSize = HlbregisterUtils.getPageSize(); // 页面大小，设置为“-1”表示不进行分页（分页无效）
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
		...
		if (StringUtils.isNotBlank(size)) {
			HlbregisterUtils.savePageSize(Integer.valueOf(size));
		}
		this.pageSize = HlbregisterUtils.getPageSize();
		// 设置排序参数
		String orderBy = request.getParameter("orderBy");
		if (StringUtils.isNotBlank(orderBy)) {
			this.setOrderBy(orderBy);
		}
		HlbregisterUtils.savePageSize(pageSize);
	}
	...
	public String toString() {
		...
		// 分页格式：第i页 共m页 共k行 行数输入选择框
		sb.append("<li class=\"disabled controls\"><a href=\"javascript:\"> ");
		sb.append(" 第").append(pageNo).append("页");
		sb.append(" 共").append(end).append("页");
		sb.append(" 共").append(count).append("行");

		sb.append(" <select style=\"width:60px;height:30px;\" onchange=\"page(" + pageNo + ",this.value)\">");
		if (pageSize == 10) {
		...
		if (pageSize == 15) {
			sb.append("<option value=\"15\" selected='selected'>15</option>");
		} else {
		...
		if (pageSize == 35) {
			sb.append("<option value=\"35\" selected='selected'>35</option>");
		} else {
			sb.append("<option value=\"35\">35</option>");
		}
		if (pageSize == 40) {
		...
		if (pageSize == 45) {
			sb.append("<option value=\"45\" selected='selected'>45</option>");
		} else {
