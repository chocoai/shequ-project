package com.haolinbang.common.service;
...
import com.haolinbang.common.persistence.BaseEntity;
...
public abstract class BaseService {
	 ...
	 * @return
	 */
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String tableAlias) {
		StringBuilder sql = new StringBuilder();

		User user = entity.getCurrentUser();
		// 进行权限过滤，多个角色权限范围之间为或者关系。
		List<String> groupDataScopeList = new ArrayList<String>();
		List<String> wyidDataScopeList = new ArrayList<String>();
		// 超级管理员，跳过权限过滤
		if (!user.isAdmin()) {
			// 查询访问范围
			List<AppUserScope> appUserScopeList = user.getAppUserScopeList();
			for (AppUserScope scope : appUserScopeList) {
				if ("G".equals(scope.getType())) {
					groupDataScopeList.add(scope.getGroupId());
				} else if ("WY".equals(scope.getType())) {
					wyidDataScopeList.add(scope.getGroupId());
				}
			}

			// 主表的別名默认为a,组织机构字段都为group_id
			if (StringUtils.isBlank(tableAlias)) {
				tableAlias = "a";
			}
			if (!groupDataScopeList.isEmpty()) {
				sql.append(" AND ").append(tableAlias).append(".group_id IN(").append(StringUtils.join(groupDataScopeList.toArray(), ",")).append(")");
			}
			if (!wyidDataScopeList.isEmpty()) {
				sql.append(" OR ").append(tableAlias).append(".wyid IN(").append(StringUtils.join(wyidDataScopeList.toArray(), ",")).append(")");
			}
		}
