...
public class WeixinServiceImpl extends WxMpServiceImpl implements WeixinService {
	...
	private WxMpMessageRouter wxMpMessageRouter = new WxMpMessageRouter(this);
	 ...
	 * @param accountId
	 */
	private void setAccountConf(String accountId) {
		WxAccount wxAccount = wxAccountService.get(accountId);
		if (wxAccount != null) {
			config.setAppId(wxAccount.getAppid()); // 设置微信公众号的appid
			config.setSecret(wxAccount.getAppsecret()); // 设置微信公众号的app
			config.setToken(wxAccount.getToken()); // 设置微信公众号的token,用来唯一标识一个微信公众号的请求
			config.setAesKey(wxAccount.getEncodingAESKey()); // 设置微信公众号的EncodingAESKey
			this.setWxMpConfigStorage(config);
		} else {
			throw new RuntimeException("设置微信账号秘钥配置信息出错,微信账号为空,accountId=" + accountId);
		}
	}
	...
	public WeJson mass(HttpServletRequest request) {
		...
		try {
			...
			uploadMediaRes = this.mediaUpload(WxConsts.MEDIA_IMAGE, WxConsts.FILE_JPG, inputStream);
			...
			WxMpMassUploadResult massUploadResult = this.massNewsUpload(news);
			...
			WxMpMassSendResult massResult = this.massOpenIdsMessageSend(massMessage);
	...
	public List<WxMaterialFileBatchGetNewsItem> getWxImages(int i, int j) throws Exception {
		setDefaultAccount();
		WxMpMaterialFileBatchGetResult wxMpMaterialFileBatchGetResult = this.materialFileBatchGet(WxConsts.MATERIAL_IMAGE, i, j);
		...
		for (WxMaterialFileBatchGetNewsItem item : items) {
			if (StringUtils.isNotBlank(item.getMediaId())) {
				...
				if (StringUtils.isNotBlank(str)) {
					...
					InputStream is = this.materialImageOrVoiceDownload(item.getMediaId());
	...
	public WxNewsArticle addUrl(WxNewsArticle wxNewsArticle) {
		...
		if (StringUtils.isNotBlank(wxNewsArticle.getThumbMediaId())) {
			...
			if (StringUtils.isNotBlank(str)) {
				...
				InputStream is;
				try {
					is = this.materialImageOrVoiceDownload(wxNewsArticle.getThumbMediaId());
					str = QRCodeUtil.encode2ImgBase64(is);
				} catch (Exception e) {
					log.error("{}", e);
				}
	...
	public WxMediaUploadResult wxImagesUpload(String filename,String accountId) throws Exception {
		...
		WxMediaUploadResult uploadMediaRes = this.mediaUpload(WxConsts.MEDIA_IMAGE, WxConsts.FILE_JPG, inputStream);
		// 将文件加到缓存中
	...
	public List<WxMaterialNewsBatchGetNewsItem> getNewsList(int i, int j) throws WxErrorException {
		// 设置默认账号
		setDefaultAccount();
		WxMpMaterialNewsBatchGetResult wxMpMaterialNewsBatchGetResult = this.materialNewsBatchGet(0, 20);
	...
	public String getBase64StrByMediaId(String thumbMediaId) throws Exception {
		...
		if (StringUtils.isNotBlank(str)) {
			...
			InputStream is = this.materialImageOrVoiceDownload(thumbMediaId);
			str = QRCodeUtil.encode2ImgBase64(is);
	...
	public List<WxMpMassSendResult> sendMassNews(WxMpMassNews news, String accountId) throws WxErrorException {
		setAccountConf(accountId);
		log.info("---------测试信息news---------：{}", JSON.toJSONString(news));
		log.info("---------测试信息accountId---------：{}", accountId);
		WxMpMassUploadResult massUploadResult = this.massNewsUpload(news);
		List<WxMpMassSendResult> wxMpMassSendResultList = new ArrayList<WxMpMassSendResult>();
		List<WxMpGroup> groupList = this.groupGet();
		...
		for (WxMpGroup group : groupList) {
			...
			WxMpMassSendResult massResult = this.massGroupMessageSend(gmsg);
			wxMpMassSendResultList.add(massResult);
		}

		log.info("群发结果:{}", wxMpMassSendResultList);
		...
		// this.massOpenIdsMessageSend(massMessage);
		// wxMpMassSendResultList.add(massResult);

		// 按照标签群发
		// WxMpMassTagMessage tagMsg = new WxMpMassTagMessage();
		// tagMsg.setMsgType(WxConsts.MASS_MSG_NEWS);
		// tagMsg.setMediaId(massUploadResult.getMediaId());
		// //
		// tagMsg.setTagId(this.getUserTagService().tagGet().get(0).getId());
		//
		// WxMpMassSendResult massResult2 =
		// this.massGroupMessageSend(tagMsg);
		// wxMpMassSendResultList.add(massResult2);
	...
	public WxMediaUploadResult wxImagesUpload(InputStream inputStream, String filename, String accountId) throws Exception {
		...
		InputStream is1 = new ByteArrayInputStream(bos.toByteArray());// 用于上传的文件流
		InputStream is2 = new ByteArrayInputStream(bos.toByteArray());// 用于保存到本地的文件流
		WxMediaUploadResult uploadMediaRes = this.mediaUpload(WxConsts.MEDIA_IMAGE, type, is1);
	...
	public Map<String, String> wxNewsImagesUpload(InputStream inputStream, String filename) throws IOException, RuntimeException, WxErrorException {
		...
		// 将图文消息进行上传
		WxMpMaterialUploadNewsImgResult result = this.uploadImg(type, inputStream);
