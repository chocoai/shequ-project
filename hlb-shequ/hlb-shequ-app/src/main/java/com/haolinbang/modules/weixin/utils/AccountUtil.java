...
public class AccountUtil {
	...
	public static WxAccount getDefaultAccount() {
		WxAccount account = CacheUtils.get(WX_ACCOUNT_CACHE, WX_ACCOUNT_CACHE_ID + UserUtils.getUser().getId());
		if (account == null) {
			account = wxAccountDao.getDefaultWxAccount();
			CacheUtils.put(WX_ACCOUNT_CACHE, WX_ACCOUNT_CACHE_ID + UserUtils.getUser().getId(), account);
		}
		return account;
	}
