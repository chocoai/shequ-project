				...
				<tr>
					...
					<td>
						<c:if test="${process.suspended}">
							<a href="${ctx}/act/process/update/active?procDefId=${process.id}" onclick="return confirmx('确认要激活吗？', this.href)">启用</a>
						</c:if>
						<c:if test="${!process.suspended}">
							<a href="${ctx}/act/process/update/suspend?procDefId=${process.id}" onclick="return confirmx('确认挂起除吗？', this.href)">暂停</a>
						</c:if>
						<a href='${ctx}/act/process/delete?deploymentId=${process.deploymentId}' onclick="return confirmx('确认要删除该流程吗？', this.href)">删除</a>                        
    					<a onclick="setHandler('${process.id}','设置处理类')" href='#' title="设置处理类">设置处理类</a>
    					<a onclick="setActForm('${process.id}','设置表单')" href='#' title="设置表单">设置表单</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
