...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/appMenu/">应用平台菜单表列表</a></li>
		<li class="active"><a href="${ctx}/sys/appMenu/form?id=${appMenu.id}">应用平台菜单表<shiro:hasPermission name="sys:appMenu:edit">${not empty appMenu.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:appMenu:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="appMenu" action="${ctx}/sys/appMenu/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">文字显示颜色：</label>
			<div class="controls">
				<form:input path="color" htmlEscape="false" maxlength="100" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">子系统系统类别编码：</label>
			<div class="controls">
				<form:input path="sysType" htmlEscape="false" maxlength="20" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
