				...
				function(a) {
					...
					var b, c = a.lang.specialChar, d = function(j) {
						...
						if (k.getName() == 'a' && (l = k.getChild(0).getHtml())) {
							if (f)
								h(null, f);
							var m = b.getContentElement('info', 'htmlPreview')
									.getElement();
							b.getContentElement('info', 'charPreview')
									.getElement().setHtml(l);
						...
						if (k.getName() == 'a') {
							b.getContentElement('info', 'charPreview')
									.getElement().setHtml('&nbsp;');
							b.getContentElement('info', 'htmlPreview')
									.getElement().setHtml('&nbsp;');
							...
							.addFunction(function(j) {
								...
								var k = j.getTarget(), l, m, n = j
										.getKeystroke(), o = a.lang.dir == 'rtl';
								switch (n) {
								case 38:
									if (l = k.getParent().getParent()
											.getPrevious()) {
										m = l.getChild([
												k.getParent().getIndex(), 0 ]);
										m.focus();
										h(null, k);
										g(null, m);
									}
								...
								case 9:
									if (l = k.getParent().getNext()) {
										...
										if (m.type == 1) {
											...
											h(null, k);
									} else if (l = k.getParent().getParent()
											.getNext()) {
										m = l.getChild([ 0, 0 ]);
										if (m && m.type == 1) {
											m.focus();
