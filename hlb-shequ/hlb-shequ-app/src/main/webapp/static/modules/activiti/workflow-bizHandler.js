    ...
    $.getJSON(ctx + '/workflow/process/trace2?pid=' + opts.pid, function(infos) {
        ...
        if ($('#workflowTraceDialog').length == 0) {
            $('<div/>', {
                ...
                html: "<div><img src='" + imageUrl + "' style='position:absolute; left:0px; top:100px;' />" +
                ...
                $('.activity-attr').click(function() {
                    ...
                    if (taskType == "用户任务") {
                        $('#selectHandler').dialog({
                            ...
                            open: function() {
                                $('#selectHandler').empty();
                                $('#selectHandler').dialog('option', 'title', '任务节点【<font color="red">' + taskName + '</font>】配置处理方法');

                                //获取选择页面
                                var src = ctx + '/sns/wyActHandler/showHandler?activitiId=' + activitiId + '&procDefinitionId=' + procDefinitionId;
