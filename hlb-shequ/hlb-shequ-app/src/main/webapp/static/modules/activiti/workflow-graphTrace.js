    ...
    $.getJSON(ctx + '/workflow/process/trace2?pid=' + opts.pid, function(infos) {
        ...
        if ($('#workflowTraceDialog').length == 0) {
            $('<div/>', {
                ...
                html: "<div><img src='" + imageUrl + "' style='position:absolute; left:40px; top:175px;' />" +
                ...
                $('.activity-attr').click(function() {
                    ...
                    if (taskType == "用户任务") {
                        $('#selectHandler').dialog({
                            ...
                            open: function() {
                                $('#selectHandler').empty();
                                $('#selectHandler').dialog('option', 'title', '任务节点【<font color="red">' + taskName + '</font>】配置待办人员');

                                var src = ctx + '/sns/wyActCandidate/selectHandler?activitiId=' + activitiId + '&procDefinitionId=' + procDefinitionId;
