...
public class FileUploadCommand extends Command implements IPostCommand {
	...
	protected void handleOnUploadCompleteCallFuncResponse(final OutputStream out, final String errorMsg, final String path) throws IOException {
		...
		out.write(("window.parent.CKFinder.tools.callFunction(" + this.ckFinderFuncNum + ", '" + path + FileUtils.backupWithBackSlash(this.newFileName, "'") + "', '" + errorMsg + "');")
				.getBytes("UTF-8"));
	}
	...
	private boolean validateUploadItem(final FileItem item, final String path) {
		...
		// 文件后缀名,通过base62将文件名加密
		String extentsion = FileUtils.getFileExtension(this.fileName);
		this.newFileName = Encodes.encodeBase62(user.getSource() + user.getId()) + DateUtils.getDateTimeyyyyMMddmmssSSSS() + "." + extentsion;
