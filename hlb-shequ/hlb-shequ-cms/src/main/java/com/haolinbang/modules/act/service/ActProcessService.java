...
public class ActProcessService extends BaseService {
	 ...
	 * 流程定义列表
	 */
	public Page<WyActDef> processList(Page<WyActDef> page, Act act) {
		ProcessDefinitionQuery processDefinitionQuery = repositoryService.createProcessDefinitionQuery().orderByProcessDefinitionCategory().desc()
				.orderByProcessDefinitionVersion().desc();
		if (StringUtils.isNotEmpty(act.getCategory())) {
			processDefinitionQuery.processDefinitionCategory(act.getCategory());
		}
		...
		List<WyActDef> list = new ArrayList<WyActDef>();
		for (ProcessDefinition processDefinition : processDefinitionList) {
			String deploymentId = processDefinition.getDeploymentId();
			Deployment deployment = repositoryService.createDeploymentQuery().deploymentId(deploymentId).singleResult();

			WyActDef wyActDef = wyActDefService.getWyActDefByProcDefId(processDefinition.getId());
			if (wyActDef != null) {
				wyActDef.setDeploymentTime(deployment.getDeploymentTime());
				wyActDef.setDiagramResourceName(processDefinition.getDiagramResourceName());
				wyActDef.setResourceName(processDefinition.getResourceName());
				wyActDef.setSuspended(processDefinition.isSuspended());
				wyActDef.setDeploymentId(deploymentId);
				list.add(wyActDef);
			}
		}
		if (processDefinitionQuery.count() > 0 && list != null && !list.isEmpty()) {
			page.setList(list);
		}
		return page;
	}
	 ...
	 * @return
	 */
	public List<Act> getProcessList(Act act) {
		ProcessDefinitionQuery processDefinitionQuery = repositoryService.createProcessDefinitionQuery().orderByProcessDefinitionCategory().desc()
				.orderByProcessDefinitionVersion().desc();
		if (StringUtils.isNotEmpty(act.getCategory())) {
			processDefinitionQuery.processDefinitionCategory(act.getCategory());
		...
		for (ProcessDefinition processDefinition : processDefinitionList) {
			Act act2 = new Act();
			act2.setProcDefId(processDefinition.getId());
			act2.setProcDefName(processDefinition.getName());
			act2.setProcDefKey(processDefinition.getKey());
			act2.setProcDefVersion(processDefinition.getVersion());
			act2.setProcDefCategory(processDefinition.getCategory());
			actList.add(act2);
		}
	...
	public String deploy(String exportDir, String category, MultipartFile file) {
		...
		try {
			...
			for (ProcessDefinition processDefinition : list) {
				...
				// 部署完成后,将数据保存到业务表中
				WyActDef wyActDef = new WyActDef();
	...
	public void updateCategory(String procDefId, String category) {
		...
		// 更新对应的维护表
		WyActDef wyActDef = wyActDefService.getWyActDefByProcDefId(procDefId);
		wyActDef.setCategory(category);
		wyActDefService.save(wyActDef);
	}
	...
	public void deleteDeployment(String deploymentId) {
		...
		for (ProcessDefinition processDefinition : list) {
			wyActDefService.deleteByProcDefId(processDefinition.getId());
		}
	 ...
	 */
	@Transactional(readOnly = false)
	public void updateInfo(String procDefId, String actAliasName, String description) {
		WyActDef wyActDef = wyActDefService.getWyActDefByProcDefId(procDefId);
		wyActDef.setAlias(actAliasName);
		wyActDef.setRemarks(description);// 备注信息
		wyActDefService.save(wyActDef);
	}
