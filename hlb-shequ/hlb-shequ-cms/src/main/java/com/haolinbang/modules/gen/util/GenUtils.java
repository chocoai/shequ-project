package com.haolinbang.modules.gen.util;
...
import com.haolinbang.common.mapper.JaxbMapper;
...
import com.haolinbang.modules.gen.entity.GenCategory;
import com.haolinbang.modules.gen.entity.GenConfig;
...
import com.haolinbang.modules.gen.entity.GenTemplate;
...
public class GenUtils {
	...
	public static void initColumnField(GenTable genTable) {
		for (GenTableColumn column : genTable.getColumnList()) {
			...
			} else if (StringUtils.containsIgnoreCase(column.getJdbcType(), "DOUBLE") && !StringUtils.containsIgnoreCase(column.getJdbcType(), "FLOAT")) {
				column.setJavaType("Double");
			} else if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "INT") || StringUtils.startsWithIgnoreCase(column.getJdbcType(), "NUMBER")
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(), "BIGINT")) {
				// 如果是浮点型
			...
			} else if (StringUtils.containsIgnoreCase(column.getJdbcType(), "TINYINT")) {
				String[] ss = StringUtils.split(StringUtils.substringBetween(column.getJdbcType(), "(", ")"), ",");
				// 数据库中tinyint类型，将其转成boolean
		...
		try {
			...
			if (file != null) {
				return file.getAbsolutePath()
						+ File.separator
						+ StringUtils.replaceEach(GenUtils.class.getName(), new String[] { "util." + GenUtils.class.getSimpleName(), "." }, new String[] { "template",
								File.separator });
			}
		} catch (Exception e) {
			logger.error("{}", e);
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme) {
		...
		model.put("permissionPrefix", model.get("moduleName")
				+ (StringUtils.isNotBlank(genScheme.getSubModuleName()) ? ":" + StringUtils.lowerCase(genScheme.getSubModuleName()) : "") + ":" + model.get("className"));
	...
	public static String generateToFile(GenTemplate tpl, Map<String, Object> model, boolean isReplaceFile) {
		...
		String fileName = Global.getProjectPath()
				+ File.separator
				+ StringUtils.replaceEach(FreeMarkers.renderString(tpl.getFilePath() + "/", model), new String[] { "//", "/", "." }, new String[] { File.separator, File.separator,
						File.separator }) + FreeMarkers.renderString(tpl.getFileName(), model);
