...
<body>
	...
	<script type="text/javascript">
		function saveCandidateSelectHandler(){
			//-------部门选择
			var orgIds = [],orgIdsName = [], orgNodes = orgTree.getCheckedNodes(true);
			for(var i=0; i<orgNodes.length; i++) {
				...
				if(!halfCheck.half && id.indexOf("G_") ==0){
					orgIds.push(id);
					orgIdsName.push(orgNodes[i].name);
				}
			}
			console.info(orgIds);			
			...
			//-------用户选择
			var candidateIds = [],candidateIdsName = [],  candidateNodes = candidateTree.getCheckedNodes(true);
			for(var i=0; i<candidateNodes.length; i++) {
				...
				if(!halfCheck.half && id.indexOf("M_") ==0){
					...
					candidateIdsName.push(candidateNodes[i].name);
				}
			}		
			...
			var orgIdsNameStr=orgIdsName.join(",");
			var candidateIdsNameStr=candidateIdsName.join(",");
			
			var selectOrgIdsOrCandidateIds ="";
			if(orgIdsName && orgIdsName.length >0){
				selectOrgIdsOrCandidateIds += "部门:" + orgIdsNameStr + ";";
			}
			if(candidateIdsName && candidateIdsName.length >0){
				selectOrgIdsOrCandidateIds += "人员:" + candidateIdsNameStr;
			}
			
			 //向父页面进行传值
			 parent.$("#selectOrgIdsOrCandidateIds").text(selectOrgIdsOrCandidateIds);
			 parent.$('#orgIds').val(orgIds);
			 parent.$('#candidateIds').val(candidateIds);			 
