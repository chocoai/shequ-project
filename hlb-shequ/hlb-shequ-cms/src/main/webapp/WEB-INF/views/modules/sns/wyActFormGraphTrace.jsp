  ...
  ProcessDiagramGenerator.options = {
    ...
    on: {
      click: function(canvas, element, contextObject){
		...
		if(contextObject.getProperty("type")=="userTask"||contextObject.getProperty("type")=="endEvent"||contextObject.getProperty("type")=="startEvent"){
			var activitiId = contextObject.id;
			
			var url="${ctx}/sns/wyActForm/showForm";			 
			var activitiType=contextObject.getProperty("type");
            /* if(activitiType=="endEvent" || activitiType=="startEvent"){
            	url="${ctx}/sns/wyActForm/showForm2";
            } */
			
			var name=contextObject.getProperty("name");
			if(!name){
				if(activitiType=="startEvent"){
					name="开始节点";
				}else if(activitiType=="endEvent"){
					name="结束节点";
				}
			}
			
		 	var src = url + '?activitiId=' + activitiId + '&procDefinitionId=' + processDefinitionId;				
			var titleName2 = "【<font color='red'>"+ name +"</font>】"+ decodeURIComponent(titleName);			
