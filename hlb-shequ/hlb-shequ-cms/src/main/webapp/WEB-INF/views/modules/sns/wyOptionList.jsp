...
<body>
	...
	<form:form id="searchForm" modelAttribute="wyOption" action="${ctx}/sns/wyOption/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns">
				<shiro:hasPermission name="sns:wyOption:edit">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>创建时间</th>
				<th>更新时间</th>
				<shiro:hasPermission name="sns:wyOption:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="wyOption">
			<tr>
				...
				<td><a>${wyOption.title}</a></td>
				<td><a>${wyOption.content}</a></td>
				<td><a>${wyOption.weight}</a></td>
				<td><fmt:formatDate value="${wyOption.createtime}" pattern="yyyy-MM-dd HH:mm:SS"/></td>
				<td><fmt:formatDate value="${wyOption.updatetime}" pattern="yyyy-MM-dd HH:mm:SS"/></td>
				<shiro:hasPermission name="sns:wyOption:edit"><td>
