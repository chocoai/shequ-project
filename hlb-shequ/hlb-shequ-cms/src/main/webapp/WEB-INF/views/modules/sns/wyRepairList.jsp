	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="wyRepair">
			<tr>
				...
				<td><fmt:formatDate value="${wyRepair.createtime}" pattern="yyyy-MM-dd HH:mm:SS"/></td>	
				<td><fmt:formatDate value="${wyRepair.updatetime}" pattern="yyyy-MM-dd HH:mm:SS"/></td>			
				<shiro:hasPermission name="sns:wyRepair:edit">
				<td>
    				<%-- <a href="${ctx}/sns/wyRepair/form?id=${wyRepair.id}">修改</a> --%>
					<a href="${ctx}/sns/wyRepair/delete?id=${wyRepair.id}" onclick="return confirmx('确认要删除该物业报修吗？', this.href)">删除</a>
				</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
