...
<body>
	...
	<form:form id="searchForm" modelAttribute="wxNewsMulti" action="${ctx}/weixin/wxNewsMulti/" method="post" class="breadcrumb form-search">
		<span>当前微信公众号名称为：<font color="red"><b>${wxNewsMulti.account.wxname}</b></font>，微信号为：${wxNewsMulti.account.wxnum}</span>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="wxNewsMulti">
			<tr>
				<td>${wxNewsMulti.keywords}</td>
				<td>${wxNewsMulti.news}</td>
				<td><a href="${ctx}/weixin/wxNewsMulti/form?id=${wxNewsMulti.id}">
					<fmt:formatDate value="${wxNewsMulti.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</a></td>
				<td>
					${wxNewsMulti.remarks}
				</td>
				<shiro:hasPermission name="weixin:wxNewsMulti:edit"><td>
