...
public class ActTaskService extends BaseService {
	...
	public Page<Act> historicList(Page<Act> page, Act act) {
		...
		HistoricTaskInstanceQuery histTaskQuery = historyService.createHistoricTaskInstanceQuery().taskAssignee(userId).finished().includeProcessVariables()
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (StringUtils.isNotBlank(histIns.getAssignee()) || "startEvent".equals(histIns.getActivityType()) || "endEvent".equals(histIns.getActivityType())) {
				...
				if ("startEvent".equals(histIns.getActivityType())) {
					List<HistoricProcessInstance> il = historyService.createHistoricProcessInstanceQuery().processInstanceId(procInsId).orderByProcessInstanceStartTime().asc()
							.list();
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefId, String businessTable, String businessId, Map<String, Object> vars) {
		Member member = MemberUtils.getCurrentMember();
		// 设置流程发起人
		identityService.setAuthenticatedUserId(String.valueOf(member.getMemberId()));
		...
		String businessKey = businessTable + ":" + businessId;
		vars.put("businessKey", businessKey);

		// 启动流程,业务key通过表名:业务主键id的形式保存到工作流中
		ProcessInstance procIns = runtimeService.startProcessInstanceById(procDefId, businessKey, vars);
