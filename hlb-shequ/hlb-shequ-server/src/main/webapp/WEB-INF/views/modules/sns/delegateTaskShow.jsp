		...
		function showStaff(menuId,data){
			...
			if(data){
				for(var i=0;i<data.length; i++){
					...
					html1 +='<div class="weui_media_box weui_media_small_appmsg">';
					html1 +='<div class="weui_cells weui_cells_access">';
					html1 +='<a href="javascript:;" class="weui_cell" onclick="delegateTask(\''+data[i].id+'\')">';
...
<form id="orgForm" name="orgForm">
	...
	<input type="hidden" name="bizId" value="${wyRelationCandidate.bizId}"/>
    ...
    <script type="text/javascript">
    	function delegateTask(memberid){
    		$("#memberid").val(memberid);
    		var formdata=$("#orgForm").serialize();
    		console.info(formdata);
    		window.location.href="${ctx}/wuye/delegateTask?"+formdata;    		    		
    	}
    </script>
