...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sns/wyRepairSettlementDetail/">物业维修核算明细列表</a></li>
		<li class="active"><a href="${ctx}/sns/wyRepairSettlementDetail/form?id=${wyRepairSettlementDetail.id}">物业维修核算明细<shiro:hasPermission name="sns:wyRepairSettlementDetail:edit">${not empty wyRepairSettlementDetail.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sns:wyRepairSettlementDetail:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="wyRepairSettlementDetail" action="${ctx}/sns/wyRepairSettlementDetail/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">装修类型：</label>
			<div class="controls">
				<form:input path="type" htmlEscape="false" maxlength="2" class="input-xlarge "/>
			</div>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="sns:wyRepairSettlementDetail:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
