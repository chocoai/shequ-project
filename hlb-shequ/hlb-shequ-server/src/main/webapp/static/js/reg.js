...
function selectTab(type) {
    if (type == 1) {
        selectType = 1;
        $("#isAssociate").val(false);
        $("#select-on-1").addClass("on");
        $("#select-on-2").removeClass("on");
        $("#mainMemberPhone-span").hide();
        $("#memberName-span").show();
        $("#m_mainSmscode-span").hide();
        $("#idNo-span").show();
    } else if (type == 2) {
        selectType = 2;
        $("#isAssociate").val(true);
        $("#select-on-2").addClass("on");
        $("#select-on-1").removeClass("on");
        $("#mainMemberPhone-span").show();
        $("#memberName-span").hide();
        $("#m_mainSmscode-span").show();
        $("#idNo-span").hide();
    }
    showreg();
}
...
$(function() {
    ...
    $("#idNo").keyup(function() {
        ...
        idcardno = idcardno.replace(/[^\.\d\w]/g, '');
        $(this).val(idcardno);
        if (idcardno && idcardno.length == 18) {
            if (isIdCard(idcardno)) {
                $("#idNo").data("isValidate", true);
                $("#idNo-duigou").show();
                $("#idNo-cha").hide();
            } else {
                $("#idNo").data("isValidate", false);
                $("#idNo-duigou").hide();
                $("#idNo-cha").show();
                layer.open({
                    content: "身份证号码错误",
    ...
    $("#regSubmit").click(function() {
        ...
        if ($("#isAssociate").val() === "true") {
            if($("#mainMemberPhone").val() == $("#mobile").val()){
                layer.open({
                    content: "关联人和业主的手机不能一致",
                    skin: 'msg',
                    time: 2
                });
                return;
            ...
            //关联会员验证吗验证
            if (!$("#mainSmscode").data("isValidate")) {
                $("#mainSmscode").focus();
                layer.open({
                    content: "请先通过业主手机验证码",
                    skin: 'msg',
            ...
            //如果身份证不为空，验证身份证号码校验是否通过
            var idno = $("#idNo").val();
            if ((idno != undefined && idno != '' && idno != null) && !$("#idNo").data("isValidate")) {
                $("#idNo").focus();
                layer.open({
                    content: "您输入的身份证号码错误",
                    skin: 'msg',
       ...
       /* //判断业主是否有选房
        if ($("#isAssociate").val() === "false"){
            var rwymc = $("#wymc").val();
            var rbuildingId = $("#buildingId").val();
            if(rwymc==null || rwymc=='' || rbuildingId==null || rbuildingId==''){
                layer.open({
                    content: "请先选择小区",
                    skin: 'msg',
                    time: 2
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.data == "success") {
                    ...
                    window.location.href = ctx + "/addRoom";
...
/* $('.mee').click(function(event) {
     $('.mee,.mtt').css('display', 'block');
 });*/
function showreg(){
    if(selectType==1 && $("#yzm").data("isValidate") && $("#piccode").data("isValidate")) {
